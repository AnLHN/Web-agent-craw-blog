...
Local/dev vẫn có file-backed auth store để setup nhanh; production compose dùng Postgres-backed auth store qua `APP_AUTH_STORE_BACKEND=postgres`. Password được hash bằng PBKDF2-HMAC-SHA256; bearer token chỉ lưu dạng hash trong auth/session store. User đầu tiên đăng ký tự nhận role `admin`, user sau nhận role `user`.
...
- CD cloud chưa bật; production/staging local có `docker-compose.production.yml`, còn deploy môi trường thật cần workflow riêng với GitHub Environments và Secrets.
